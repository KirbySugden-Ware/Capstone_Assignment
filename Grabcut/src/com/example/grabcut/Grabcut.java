...
public class Grabcut extends Activity implements OnTouchListener {
	...
	Mat dst = new Mat();
	final String pathToFolder = Environment.getRootDirectory().getPath(); 
	final String pathToImage =  pathToFolder + "/GCUT/gcut.png";
	...
	public boolean onTouch(View v, MotionEvent event) {
		if (event.getAction() == MotionEvent.ACTION_DOWN){
			if (counter == 0){
				...
				try{
					background = Utils.loadResource(getApplicationContext(), R.drawable.facey_pic);
				} catch(IOException e){
					e.printStackTrace();
				}
				
				backgroundSubtracting(img, background);
				Highgui.imwrite(pathToFolder + "/rect.png", dst);
				Bitmap jpg = BitmapFactory.decodeFile(pathToFolder + "/rect.png");
	...
	private void backgroundSubtracting(Mat img, Mat background) {
		...
		Imgproc.cvtColor(foreground, mask, 6);
		Imgproc.threshold(mask, mask, 254, 255, 1);
